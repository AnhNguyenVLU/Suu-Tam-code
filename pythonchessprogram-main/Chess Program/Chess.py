...
class Board(tk.Frame):
    ...
    def __init__(self, parent, length, width): #self=Frame, parent=root
        ...
        self.squares = {} #lưu trữ ô vuông với vị trí là khóa và nút
        self.ranks = string.ascii_lowercase
        self.white_images = {} #lưu trữ hình của mảnh
        self.black_images = {}
        self.white_pieces = ["pyimage1", "pyimage3", "pyimage4", "pyimage5", "pyimage6", "pyimage7"] #gọi các mảnh màu trắng
        self.black_pieces = ["pyimage8", "pyimage10", "pyimage11", "pyimage12", "pyimage13", "pyimage14"]#gọi các mảnh màu đen
        self.buttons_pressed = 0
        self.turns = 0
        self.sq1 = None #ô vuông đầu đã được nhấp
        self.sq2 = None 
        self.sq1_button = None #button liên kết hình vuông được nhấp
    ...
    def select_piece(self, button): #khi một nút vuông được nhấn, bao gồm mã chuyển động
        if button["image"] in self.white_pieces and self.buttons_pressed == 0: #kiểm tra màu sắc
            self.piece_color = "white"
        elif button["image"] in self.black_pieces and self.buttons_pressed == 0:
            self.piece_color = "black"      
        
        if (self.piece_color == "white" and self.turns % 2 == 0) or (self.piece_color == "black" and self.turns % 2 == 1) or self.buttons_pressed == 1: #chặn di chuyển quân cờ khi chưa tới lượt
